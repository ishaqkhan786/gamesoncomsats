  ...
  return (
    <>
      {session.user.role === "admin" && (
        <div className="flex items-center  justify-center lg:justify-between w-full lg:w-fit my-6 lg:my-4  lg:p-4 bg-blue-50 lg:m-4 rounded-md">
          <div className="hidden lg:flex flex-col items-center justify-center w-fit">
            <h2 className="font-bold text-2xl text-primary-500">
              Create Event for Comsats
            </h2>
            <p className="text-slate-600 text-sm font-semibold px-3 text-center mt-3">
              The event is set to occur at Comsats University Islamabad, with a
              stipulated number of teams required to reserve their slots in
              advance to participate in the event proceedings.
            </p>
              ...
              <div className="border-t border-gray-200">
                <dl>
                  <div className="bg-gray-50 text-xs px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt className="text-xs font-medium text-center text-gray-500">
                      Registration Deadline
                    </dt>
                    <dd className="mt-1 text-xs text-gray-900 sm:col-span-2">
                      All teams must register before a specified deadline to
                  ...
                  <div className="bg-white px-4 text-xs py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd className="mt-1 text-xs text-gray-900 sm:col-span-2">
                      Each team must consist of a minimum and maximum number of
                      members.
                    </dd>
                  </div>
                  <div className="bg-gray-50 px-4 text-xs py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd className="mt-1 text-xs text-gray-900 sm:col-span-2">
                      A non-refundable entry fee is required from each
                      participating team.
                    </dd>
                  </div>
                  <div className="bg-white px-4 text-xs py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd className="mt-1 text-xs text-gray-900 sm:col-span-2">
                      Teams must adhere to specific equipment regulations.
                    </dd>
                  </div>
                  <div className="bg-gray-50 px-4 text-xs py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt className="text-xs font-medium text-center text-gray-500">
      ...
      <div className="flex flex-col items-center justify-center m-3 p-4  pt-6 w-[97%]">
        <h3 className="text-2xl bg-blue-50 w-full flex items-center justify-center p-3 font-bold">
          Comsats Events
        </h3>
        <div
          className=" grid w-full grid-cols-1 mt-6 gap-6
                "
        >
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-start justify-center">
                  {session.user.role === "admin" ? (
                    <Link
                      className="px-6 mb-2 bg-primary-500 text-white rounded-full py-2"
                      href={`/events/${event._id}/teams`}
                    >
                      Check Teams
                    </Link>
                  ) : event.totalTeams > event.teams.length &&
                    session.user.role === "dptCoordinator" ? (
                    <Link
                      href={`/events/${event._id}`}
                      className="px-8 mb-2 bg-primary-500 text-white rounded-full py-2"
                    >
                      Register
                    </Link>
                  ) : (
                  ...
                  <p className="text-salte-600 font-medium text-sm">
                    Registered Teams:{" "}
                    <span className="font-lg text-primary-500 font-semibold">
                      {event.teams.length}
                    </span>
                  </p>
                </div>
