      ...
      {session.user && session.user.role === "admin" ? (
        <div className="flex flex-col items-start justify-center w-full mr-5 p-6 rounded-md bg-blue-50 pb-10 ">
          <div className="flex items-center justify-between w-full py-2 ">
            ...
            <AlertDialog>
              <AlertDialogTrigger className=" py-2 inline-flex items-center text-sm mt-2 bg-primary px-4 rounded-md text-white">
        ...
        <div className=" w-full p-4 flex-col flex items-center justify-center gap-4  ">
          ...
          {equipments.length === 0 && (
            <p className=" flex items-center justify-center w-full pb-5 text-primary font-semibold ">
              No Equipment Added
            </p>
          )}
          <div className=" grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:gap-6 ">
            {equipments.map((item) => (
              <div
                key={item._id}
                className="flex flex-col items-center justify-center bg-white   shadow-lg p-4 rounded-md mb-8"
              >
